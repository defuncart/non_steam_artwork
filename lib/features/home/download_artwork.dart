...
class DownloadArtwork extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: switch (state) {
          AsyncData(:final value) => TextField(
            decoration: InputDecoration(hintText: value.searchTerm),
      ...
      body: switch (state) {
        AsyncData(:final value) =>
          value.programResults.isEmpty
              ? Center(child: Text(context.l10n.homeProgramsEmpty))
...
class HoverableWidget extends StatefulWidget {
  const HoverableWidget({required this.child, super.key});
...
class _HoverableWidgetState extends State<HoverableWidget> {
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: Opacity(opacity: _isHovering ? 0.6 : 1, child: widget.child),
    );
  }
}
