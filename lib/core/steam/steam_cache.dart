  ...
  Future<List<SteamGridCacheProgram>> getCachePrograms() async {
    ...
    if (await dir.exists()) {
      ...
      final mapped = groupBy(
        contents,
        (file) => idRegEx.firstMatch(p.basenameWithoutExtension(file.path))?.group(0) ?? '',
      );
      ...
      for (final kvp in mapped.entries) {
        ...
        final background = kvp.value.firstWhereOrNull((file) => p.basenameWithoutExtension(file.path).length == 10);
